  ...
  if (isSupabaseConfigured()) {
    ...
    if (authData.user) {
      const [notesResult, coursesResult, categoriesResult] = await Promise.all([
        ...
        supabase.from("courses").select("id, name").eq("user_id", authData.user.id).is("archived_at", null).order("name"),
        supabase.from("note_categories").select("id, name").eq("user_id", authData.user.id).is("archived_at", null).order("name"),
      ]);
      ...
      courses = (coursesResult.data ?? []).map((row: { id: unknown; name: unknown }) => ({ id: String(row.id), name: String(row.name) }));
      categories = (categoriesResult.data ?? []).map((row: { id: unknown; name: unknown }) => ({ id: String(row.id), name: String(row.name) }));
    }
  }
