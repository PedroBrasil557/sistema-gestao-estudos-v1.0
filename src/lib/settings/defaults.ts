...
export function mapSettingsRow(row: Record<string, unknown> | null | undefined): UserSettings {
  ...
  return {
    ...
    sidebarColor: String(row.sidebar_color ?? defaultUserSettings.sidebarColor),
    buttonColor: String(row.button_color ?? defaultUserSettings.buttonColor),
    cardTone: (["soft", "neutral", "vivid"].includes(String(row.card_tone)) ? String(row.card_tone) : defaultUserSettings.cardTone) as UserSettings["cardTone"],
